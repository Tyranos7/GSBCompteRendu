...
-- XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
set names 'utf8';
create table `MOTIF`(`MOT_CODE` varchar(5) not null, `MOT_LIBELLE` varchar(255),primary key (`MOT_CODE`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table `PRATICIEN`(`PRA_NUM` SMALLINT not null,`PRA_NOM` VARCHAR(25),`PRA_PRENOM` VARCHAR(30),`PRA_ADRESSE` VARCHAR(50),`PRA_CP` VARCHAR(5),`PRA_VILLE` VARCHAR(25),`PRA_COEFNOTORIETE` FLOAT, `PRA_COEFCONFIANCE` FLOAT,`TYP_CODE` VARCHAR(3) not null,primary key(`PRA_NUM`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table `RAPPORT_VISITE`(`COL_MATRICULE` VARCHAR(10) not null,`RAP_NUM` INT not null,`PRA_NUM` SMALLINT not null, `PRA_NUM_REMPLACANT` SMALLINT,`RAP_DATE_VISITE` DATE, `RAP_DATE_SAISIE` DATE,`RAP_BILAN` VARCHAR(255),`RAP_MOTIF_AUTRE` VARCHAR(255),`MOT_CODE` VARCHAR(5) not null,`RAP_DOCUMENTATION` TINYINT(1) not null,`RAP_SAISIEDEFINITIVE` TINYINT(1) not null,primary key(`COL_MATRICULE`,`RAP_NUM`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table `TRAVAILLER`(`COL_MATRICULE` VARCHAR(10) not null,`DATE_DEBUT` DATE not null,`REG_CODE` VARCHAR(2) not null, `DATE_FIN` DATE, primary key(`DATE_DEBUT`,`COL_MATRICULE`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
create table `COLLABORATEUR`(`COL_MATRICULE` VARCHAR(10) not null,`COL_NOM` VARCHAR(25),`COL_PRENOM` VARCHAR(50),`COL_ADRESSE` VARCHAR(50),`COL_CP` VARCHAR(5),`COL_VILLE` VARCHAR(30),`COL_DATEEMBAUCHE` DATE,`COL_LOGIN` VARCHAR(20),`COL_MDP` VARCHAR(15),primary key(`COL_MATRICULE`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
create table `RESPONSABLE`(`RES_MATRICULE` VARCHAR(10) not null,`SEC_CODE` VARCHAR(1),`RES_DATENOMINATION` DATE not null,primary key(`RES_MATRICULE`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
create table `DELEGUE_REGIONAL`(`DEL_MATRICULE` VARCHAR(10) not null, `REG_CODE` VARCHAR(2),`DEL_DATEPROMOTION` DATE not null,primary key(`DEL_MATRICULE`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
create table `PRESENTER`(`MED_DEPOTLEGAL` varchar(10) not null, `COL_MATRICULE` varchar(10), `RAP_NUM` INT not null,primary key (`MED_DEPOTLEGAL`, `COL_MATRICULE`, `RAP_NUM`)) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
alter table `RESPONSABLE` add foreign key (`RES_MATRICULE`) references `COLLABORATEUR`(`COL_MATRICULE`);
alter table `RESPONSABLE` add foreign key (`SEC_CODE`) references `SECTEUR`(`SEC_CODE`);
alter table `DELEGUE_REGIONAL` add foreign key (`DEL_MATRICULE`) references `COLLABORATEUR`(`COL_MATRICULE`);
alter table `DELEGUE_REGIONAL` add foreign key (`REG_CODE`) references `REGION`(`REG_CODE`);
alter table `RAPPORT_VISITE` add foreign key (`PRA_NUM`) references `PRATICIEN`(`PRA_NUM`);
alter table `RAPPORT_VISITE` add foreign key (`PRA_NUM_REMPLACANT`) references `PRATICIEN`(`PRA_NUM`);
...
alter table `RAPPORT_VISITE` add foreign key (`MOT_CODE`) references `MOTIF`(`MOT_CODE`);
...
alter table `PRESENTER` add foreign key (`MED_DEPOTLEGAL`) references `MEDICAMENT` (`MED_DEPOTLEGAL`);
alter table `PRESENTER` add foreign key (`COL_MATRICULE`,`RAP_NUM`) references `RAPPORT_VISITE`(`COL_MATRICULE`,`RAP_NUM`);
